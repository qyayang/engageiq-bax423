...
def _safe_text(value) -> str:
    """Convert missing/NaN CSV values into empty strings before text operations."""
    if value is None:
        return ""
    try:
...
def compute_effort_score(row: dict) -> float:
    ...
    if source == "github":
        ...
        words = len(_safe_text(row.get("description")).split())
...
def compute_trend_score(row: dict) -> float:
    ...
    if _safe_text(row.get("source")) == "hackernews":
...
def _intent_bonus(row: dict, intent: str | None) -> float:
    ...
    if intent == "contribution":
        ...
        # GitHub repos are the target — boost repo health, hard-penalise HN at stage-1
        if source == "github":
            community = compute_community_health(row)
            bonus += 0.12 * community
            contrib = max(row.get("contributors", 1), 1)
            stars   = max(row.get("stars", 0), 0)
            gap     = min(stars / contrib / 1000, 1.0)
            bonus  += 0.06 * gap  # niche/underserved projects
        elif source == "hackernews":
            bonus -= 0.25  # maintainers/DevOps work lives on GitHub, not HN
        ...
        # Boost: GitHub issues, security keywords, GFI for beginner-accessible vulns
        if source == "github":
            bonus += 0.08
        if row.get("record_type", "") == "issue":
            bonus += 0.08
        if row.get("good_first_issues", 0) > 0:
            bonus += 0.06
        text = f"{_safe_text(row.get('title'))} {_safe_text(row.get('description'))}".lower()
        if any(kw in text for kw in ("security", "vulnerability", "vuln", "cve", "audit", "exploit", "auth", "owasp")):
            bonus += 0.15
...
def compute_composite_score(
    ...
    intent_override bypasses ROLE_INTENT lookup — use for hidden/inferred personas.
    """
    community   = compute_community_health(row)
    ...
    intent    = intent_override if intent_override is not None else ROLE_INTENT.get(persona, None)
    composite = (w_rel * relevance + w_com * community + w_vis * visibility
...
def suggest_actions(row: dict, persona: str = "", intent_override: str | None = None) -> list[str]:
    """Specific, rule-based next actions for the recommendation card."""
    source = row.get("source", "")
    domain = row.get("domain", "")
    record_type = row.get("record_type", "")
    title = _safe_text(row.get("title"))
    text = f"{title} {_safe_text(row.get('description'))}".lower()
    intent = intent_override if intent_override is not None else ROLE_INTENT.get(persona)
    actions = []
